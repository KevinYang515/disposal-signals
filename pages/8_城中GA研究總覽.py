...
def _apply_influence_quartile_8(q_idx):
    if q_idx is None:
        lo, hi = 0.0, 100.0
    else:
        lo, hi = float(_infl_quartile_bounds_8[q_idx]), float(_infl_quartile_bounds_8[q_idx + 1])
    for k, v in (('influence_lo_8_slider', lo), ('influence_lo_8_num', lo),
                 ('influence_hi_8_slider', hi), ('influence_hi_8_num', hi)):
        st.session_state[k] = v


q_cols_8 = st.columns(5)
for _i, _label in enumerate(['Q1(最低)', 'Q2', 'Q3', 'Q4(最高)']):
    with q_cols_8[_i]:
        st.button(_label, key=f'infl_q{_i+1}_btn_8', on_click=_apply_influence_quartile_8, args=(_i,))
with q_cols_8[4]:
    st.button('全部(不篩選)', key='infl_qall_btn_8', on_click=_apply_influence_quartile_8, args=(None,))
st.caption(f'快速套用四分位邊界（依目前資料重算）：Q1<{_infl_quartile_bounds_8[1]:.2f}% | Q2<{_infl_quartile_bounds_8[2]:.2f}% | Q3<{_infl_quartile_bounds_8[3]:.2f}% | Q4≤{_infl_quartile_bounds_8[4]:.2f}%。')
...
base_mask &= events['cz_influence_pct'].between(influence_lo, influence_hi)
...
stop_tp_mode = st.radio(
    '停損／停利計算方式', ['百分比(%)', '距漲跌停 Ticks'], key='stop_tp_mode_8', horizontal=True,
...
if stop_tp_mode == '百分比(%)':
    ...
    with col_t1:
        stop_n_ticks = st.number_input('停損：距D1理論漲停幾個tick出場（0=不停損）', min_value=0, max_value=50, value=0, step=1, key='stop_ticks_8')
    with col_t2:
        tp_n_ticks = st.number_input('停利：距D1理論跌停幾個tick出場（0=不停利）', min_value=0, max_value=50, value=0, step=1, key='tp_ticks_8')
...
with np.errstate(invalid='ignore'):
    if stop_tp_mode == '距漲跌停 Ticks':
        limit_up_arr = tick_rounded_limit_up(d0_close_arr)
        limit_down_arr = tick_rounded_limit_down(d0_close_arr)
        ...
        hit_stop = (stop_n_ticks > 0) & np.isfinite(stop_price) & (high_arr >= stop_price)
        hit_tp = (tp_n_ticks > 0) & np.isfinite(tp_price) & (low_arr <= tp_price)
    else:
...
if len(settled) == 0:
    ...
    if stop_pct > 0 or tp_pct > 0 or stop_n_ticks > 0 or tp_n_ticks > 0 or addon_enabled:
        if stop_tp_mode == '距漲跌停 Ticks':
            kpi_note = f'停損距漲停{stop_n_ticks}tick / 停利距跌停{tp_n_ticks}tick'
        else:
