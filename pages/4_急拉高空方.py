...
# ── 停損失效風險：已修正 ──────────────────────────────────
st.subheader("✅ 停損失效風險（2026-07-13已修正）")
if "stop_above_limit" in sub.columns:
    ...
    st.success(
        ...
        f"**投組層級全樣本影響**：Q10 Sharpe 2.99→3.45、累積+1981%→+3036%、MaxDD -20.0%→-18.4%；"
        f"Q8 Sharpe 3.10→3.58、累積+2301%→+3641%、MaxDD -21.7%→-18.4%。"
        f"（現在上方指標卡與資料已是修正後版本；VM live腳本已同步套用此修正。）"
    )
else:
...
**停損**：09:00-09:30累積最高點 + 1 tick，**並夾在漲停價-1 tick以下**（2026-07-13修正，
見下方「✅停損失效風險」段落）
