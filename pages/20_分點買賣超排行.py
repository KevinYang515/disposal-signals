# -*- coding: utf-8 -*-
r"""分點買賣超排行查詢：自選分點(可多選)、自選截止日、自選往前幾個交易日、自選金額門檻，
看這些分點在這段期間對各股票的估計淨買賣超金額排行(多選時會合併加總，並標出有幾個
選定分點同時出現在同一檔股票，方便看「共振」)。
...
with tab_pos:
    st.markdown("#### 選定分點(可多選)，目前還抱著哪些股票、布局到目前賺多少")
    st.caption(
        ...
        "多選時各分點的持股分開列出(不加總)，同一檔股票若被多個分點同時持有會分別出現，方便看誰跟誰持股重疊。"
    )
    if positions.empty:
        st.info("找不到 `data/broker_positions.parquet`，這個分頁的資料尚未產生（需要重新跑一次每日產生器）。")
    else:
        pos_brokers = sorted(positions["broker"].unique().tolist())
        pos_default = ["凱基-站前"] if "凱基-站前" in pos_brokers else pos_brokers[:1]
        sel_pos_brokers = st.multiselect("分點（可多選、可直接打字搜尋）", pos_brokers, default=pos_default, key="pos_broker")
        if not sel_pos_brokers:
            st.info("請至少選一個分點。")
        else:
            pv = positions[positions["broker"].isin(sel_pos_brokers)].copy()
            if pv.empty:
                st.info("所選分點目前沒有偵測到還在抱著的部位。")
            else:
                pm1, pm2, pm3, pm4 = st.columns(4)
                pm1.metric("目前持有檔數(含重複)", f"{len(pv)} 檔")
                pm2.metric("未實現損益合計", f"{pv['unrealized_wan'].sum():,.0f} 萬")
                pm3.metric("其中正報酬檔數", f"{(pv['unrealized_pct'] > 0).sum()} / {len(pv)}")
                overlap = pv.groupby("stock_id")["broker"].nunique()
                pm4.metric("多分點同時持有的股票數", f"{(overlap >= 2).sum()} 檔", delta_color="off",
                           help="有幾檔股票被2個以上所選分點同時列為目前持倉")
                show_pos = pv.rename(columns={
                    "broker": "分點", "stock_id": "代號", "name": "名稱", "episode_start": "本波起始日",
                ...
                }).sort_values("未實現損益_萬", ascending=False)
                st.dataframe(
...
with tab_flow:
    ...
    st.caption(
        ...
        "多選分點時，排行榜金額會合併加總，並標出「幾個分點同時買賣超」，方便看多分點共振。"
    )

    brokers = sorted(flow["broker"].unique().tolist())
    default_brokers = ["凱基-站前"] if "凱基-站前" in brokers else brokers[:1]
    dates_desc = sorted(flow["date"].dt.date.unique().tolist(), reverse=True)

    c1, c2, c3, c4 = st.columns([3, 2, 2, 2])
    with c1:
        sel_brokers = st.multiselect("分點（可多選、可直接打字搜尋）", brokers, default=default_brokers)
    with c2:
    ...
    broker_label = "、".join(sel_brokers) if len(sel_brokers) <= 3 else f"{len(sel_brokers)}個分點"

    view = flow[(flow["broker"].isin(sel_brokers)) & (flow["date"] >= win_start) & (flow["date"] <= end_ts)]
    if view.empty:
        st.info(f"{broker_label} 在 {win_start.date()} ~ {sel_end} 這段期間沒有 ≥ 500 萬的買賣超紀錄。")
        st.stop()
    ...
    m4.metric("涉及股票數", f"{len(agg)} 檔" + (f"（{(agg['出現分點數']>=2).sum()}檔多分點共振）" if len(sel_brokers) >= 2 else ""))
    ...
    st.caption(
        ...
        + ("**出現分點數 ≥ 3 才是這個研究裡驗證過『可跟單』的共振門檻(PLAYBOOK §6c/§6d)，1-2個分點同時出現接近雜訊。**"
           if len(sel_brokers) >= 2 else "")
    )

    fmt_cols = {"淨買賣超金額_萬": "{:,.0f}", "淨張數": "{:,.0f}", "出現分點數": "{:,.0f}",
                "期間影響力_pct": "{:+.2f}%", "期間佔股本比_pct": "{:+.3f}%"}

    st.divider()
    col_b, col_s = st.columns(2)
    with col_b:
        st.markdown(f"#### 🟥 淨買超前 25（{broker_label}）")
    ...
    with col_s:
        st.markdown(f"#### 🟩 淨賣超前 25（{broker_label}）")
    ...
    st.divider()
    st.markdown("#### 📋 完整明細（所選分點該期間所有 ≥ 500 萬紀錄，含當日影響力%／佔股本比%）")
    ...
    st.download_button(
        ...
        f"{broker_label}_{win_start.date()}_{sel_end}.csv", "text/csv", key="dl_flow",
    )
