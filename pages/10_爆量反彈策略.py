...
with tab_q1:
    ...
    st.markdown(
        ...
        "- 但「跌停鎖死」這個機制本身會再加成一次——它代表賣單在跌停價位強制排隊、"
...
with tab_ml:
    if not ML_AVAILABLE:
        ...
        st.markdown("### 獨立複核（另一顆模型 Codex 跑的）：可成交性與新因子")
        st.markdown(
            "找了另一個獨立的agent(Codex，沒看過這裡任何分析過程)重新複核，用同一套訊號重新驗證，"
            "並嘗試找新因子。結論：**沒有新因子能通過增量檢驗**，但抓到一個比找新因子更重要的執行細節。"
        )
        st.markdown(
            "**帳面報酬 vs 實際可成交報酬**：Codex 用「事件日或下一個未鎖死日的開盤價」進出場重算一次"
            "（更保守的假設：收盤才看到訊號，最早能進場是隔天開盤），TEST期均值從帳面13.08%掉到7.84%。"
            "但這個假設可能太保守——因為**只有真的當天鎖死才需要順延到隔天**，若當天沒鎖死、"
            "尾盤(收盤前最後幾分鐘)價位通常已經很接近收盤價，可以在尾盤直接進場。"
            "改用「沒鎖死就用當天收盤價(≈尾盤)，鎖死才順延到解鎖日開盤」重算，均值回到13.56%、勝率90.5%，"
            "幾乎跟帳面數字一樣。**結論：不用真的等到隔天開盤，尾盤觀察到訊號成立就可以進場試單，"
            "唯一要小心的是那約18.5%當天真的鎖死的訊號——這種不管幾點下單都成交不到，只能放棄或等解鎖。**"
        )
        st.markdown(
            "**新因子測試（融券回補/券資比/下跌加速度/事前量能/同業相對抗跌）：全部沒有通過增量檢驗**，"
            "SHAP聯合模型排名都在後段，GA把新門檻收斂到幾乎不設限。同業相對抗跌(這次崩盤跌得比同題材"
            "同業輕的股票)是最接近有效的候選——即使把台股細產業分類表擴充到561檔、24個題材(欄位"
            "`stock_id/name/theme/confidence`，存於`data/burst_ml_ga/tw_industry_theme_map.csv`，"
            "可供其他策略共用參照)重新測過，單變量分組確實變乾淨(TRAIN最低到最高五分位6.3%→13.0%)，"
            "但GA增量測試門檻只收斂到「排除最弱2.7百分位」等於幾乎沒篩選，TEST事件層級t值反而變差，"
            "仍判定證據不足，暫不採用，留作未來有更多樣本外資料再重驗的候選欄位。"
        )
