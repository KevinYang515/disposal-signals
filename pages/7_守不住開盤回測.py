...
核心邏輯（V8：V7 + aor 放寬至 3.0%）：
  個股 gap up + 漲不動 <3.0% + morning_high 距漲停 ≥1%
  + 09:35–09:37 從早盤高回落 0.05% → 空
  + Trail stop（價格新低時停損下移）+ 未觸發 11:30 強制平倉

最終配置（2026-06-28，V8 升級）：
  → CAGR +116.6%、Sharpe 29.28、MaxDD ~0%、WR 99.9%
  Walk-forward TEST Sharpe 29.65（>TRAIN 29.10）
  漲停鎖死真實風險 0%
  Slippage 0.05% 估計後仍 Sharpe ~28（世界級）
"""
...
with st.sidebar:
    ...
    st.divider()
    st.caption(
        "V8 策略：gap up + 漲不動<3.0% + 漲停安全 + **09:35–09:37 從早盤高回落 0.05%** → 空\n\n"
