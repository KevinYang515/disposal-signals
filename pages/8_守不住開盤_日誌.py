...
with st.sidebar:
    ...
    st.divider()
    st.caption(
        "**WINNER (2026-07-01)**: 從 350+ 變體 grid 找到唯一穩健配置\n\n"
        "- **Gap 2-3%**（原 0.5-10%）\n"
        "- **進場窗 09:35-11:00**（原 09:35-09:37）\n"
        "- **09:00-09:35 累計成交額 >= 5000 萬**\n"
        "- **09:35 開棒 Open >= 觸價**（過濾已破股票）\n"
        "- 其餘同原版\n\n"
        "全期 Sharpe 4.46 / CAGR +1.23% / MaxDD -0.59%\n"
        "TEST ≥2025 Sharpe 2.80，但 2025 年度負回報"
    )
...
st.warning(
    "📊 **本頁已更新為 WINNER 訊號（2026-07-01）**\n\n"
    "原版 V8 有 bug（97% 訊號實盤不可成交），已從 350+ 變體 grid 找到穩健配置。"
    "此頁面顯示的 **98 筆**是 WINNER 配置在 2023-2026 H1 的實際可執行訊號。\n\n"
    "- 全期 **Sharpe 4.46**, CAGR **+1.23%**, MaxDD **-0.59%**\n"
    "- Walk-forward TEST ≥2025 Sharpe 2.80\n"
    "- **2025 年度負回報**（低波動 regime）\n\n"
    "⚠️ CAGR 太小，實盤 slippage 可能吃光 edge。仍為模擬觀察階段。"
