...
with tab1:
    ...
    if sig.empty:
        ...
        # 注意：這裡不能用 st.stop()——Streamlit multipage app 是整個檔案由上到下線性執行，
        # st.stop() 會直接終止整支script，害後面 tab_5m/tab_t20/tab2~tab6/tab_lab 全部變空白
        # （2026-08-18 實際發生過：Tab1當天0筆導致全站除了Tab1警告外其他分頁都渲染不出來）。
        # 讓程式繼續往下跑，靠 sig 已經是正確欄位的空表自然渲染出「查無資料」的空表格即可。
    ...
    GRADE_COLOR = {
        ...
        '⬜ 待觀察':         '#95a5a6',
        '❌ 避開':           '#e74c3c',
        '🆕 新制觀察中(第一次,對照舊5分)':  '#3498db',
        '🆕 新制觀察中(第二次+,對照舊20分)': '#2980b9',
    }
    ...
    with col_f1:
        filter_grade = st.multiselect(
            ...
            options=['✅ 主力訊號', '⚠️ 漲多但大戶減碼', '🟡 觀察中', '⬜ 待觀察', '❌ 避開',
                     '🆕 新制觀察中(第一次,對照舊5分)', '🆕 新制觀察中(第二次+,對照舊20分)'],
    ...
    display_cols = ['評級', '買進訊號', '代號', '名稱', '規模', '處置原因', '處置次別', '近20日漲幅', '大戶(%)',
...
🆕 新制觀察中 = 2026/8/10 處置新制（撮合統一改2分鐘、期間縮為5~7個營業日）下的事件。舊制「主力訊號」等評級是用20分鐘撮合、10天期間驗證出來的，新制撮合頻率不同、樣本還不夠，暫不套用舊制信心度，先單純觀察累積資料。
　－ 標「對照舊5分」= 第一次處置（收款門檻：單筆10張或累積30張以上才全額收款，跟舊制5分鐘文字逐字相同，只有撮合頻率從5分變2分）
　－ 標「對照舊20分」= 第二次(含)以上處置（收款門檻：所有投資人全額收款，跟舊制20分鐘文字逐字相同，只有撮合頻率從20分變2分——這層是本策略主力訊號的原始樣本，撮合變快10倍，是最需要觀察是否還有效的一組）

更完整的新制專屬統計（依第一次/第二次+拆開的累積勝率、觸發分析等）見「處置新制觀察」頁面。
""")
