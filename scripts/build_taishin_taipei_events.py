...
follows data/fubon_branch_events.csv, with the reviewed D1 borrow-availability
flag added for this study.
"""
...
# This follows the branch-level schema used by fubon_branch_events.csv, with
# one extra reviewed D1 day-trade-short suspension audit flag for this study.
FIELDS = [
...
def main() -> None:
    ...
    for event in source.itertuples(index=False):
        ...
        ceiling_count += int(entry_is_ceiling)
        ...
        rows.append({
            ...
            # A D1 opening at its tick-rounded limit-up ceiling cannot be
            # credited as an executable opening short.  Censor it rather than
            # roll it forward to an unproven later entry, as for a fully frozen
            # D1 in the other branch event builders.
            "censored": bool(frozen or entry_is_ceiling),
    ...
    # These are informational flags in the CSV.  Page 17 applies the reviewed
    # D1 disposal and D1 borrow-availability gates as hard, non-toggleable gates.
    before = len(events)
    ...
    print(
        f"Ceiling-entry correction: {ceiling_count}/{len(events)} rows censored "
        "because their D1 open is at the tick-rounded limit-up ceiling"
    )
    print(f"Censored total: {len(events) - settled_before}/{len(events)}")
